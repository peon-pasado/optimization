...
vector<int> greedy_sp_l2() {
    ...
    for (int i=1; i<=n; ++i) {
        double val = 1e13;
        for (auto jt : at[t]) {
            if (jt->no[0][0] > n+1) continue;
            if (vis[jt->i]) continue;
            if (jt->value[0][0][0] < val) {
                val = jt->value[0][0][0];
                v = jt->i;
            }
        }        
...
vector<int> upper_bound_refine(vector<int>& quasi_solution, int lim=12, bool ford=1, bool L2 = 1) { //FORDWARD
    ...
    if (N2 <= lim) { // proceso de programación dinámica
        ...
        vector<vector<int>> dp(N1 + 1, vector<int>(subsetSize, INF));
        ...
        for (int i = 1; i <= N1; ++i) {
            int T = sum_prefix[i];
            int candidate = dp[i - 1][0] + f(fixed_item[i - 1], T);
            if (candidate < dp[i][0]) {
                dp[i][0] = candidate;
        ...
        for (int s = 1; s < subsetSize; ++s) {
            ...
            int candidate = dp[0][s ^ (1 << bit)] + f(free_item[bit], T);
            if (candidate < dp[0][s]) {
                dp[0][s] = candidate;
                tr[0][s] = bit;
            }
        ...
        for (int i = 1; i <= N1; ++i) {
            for (int s = 1; s < subsetSize; ++s) {
                ...
                int candidate = dp[i - 1][s] + f(fixed_item[i - 1], T);
                if (candidate < dp[i][s]) {
                    dp[i][s] = candidate;
                    tr[i][s] = -i;
                }
...
inline double calc_cost_lb(const vector<int>& s, const vector<double>& mu) {
    double sum = 0;
    int t = 0;
    for (auto i : s) {
        t += p[i];
...
void shrink(bool bck=1, bool repeat=0) {
    ...
    auto can_erase = [bck](Node* it) {
        ...
        if (it->no[0][0] > n+1) return 1;
        if (bck && it->no[1][0] > n+1) return 1;
...
int fs = 0;
void four_sucessive_dominance(int K=4) {
    //int mm = 0;
...
inline void update(int id, Node* no, int i, int j, double v){
    if (v > 1e9) return;
    if (no->no[id][0] == i) {
        if (no->value[id][0][0] >= v) {
            no->value[id][0][1] = no->value[id][0][0];
            no->sno[id][0][1] = no->sno[id][0][0];

            no->value[id][0][0] = v;
            no->sno[id][0][0] = j;
        } else if (no->value[id][0][1] > v) {
            no->value[id][0][1] = v;
            no->sno[id][0][1] = j;
        }
    } else if (no->no[id][1] == i) {
        if (no->value[id][1][0] >= v) {
            no->value[id][1][1] = no->value[id][1][0];
            no->sno[id][1][1] = no->sno[id][1][0];

            no->value[id][1][0] = v;
            no->sno[id][1][0] = j;
        } else if (no->value[id][1][1] > v) {
            no->value[id][1][1] = v;
            no->sno[id][1][1] = j;
        }

        if (no->value[id][0][0] > no->value[id][1][0] || 
            (abs(no->value[id][0][0] - no->value[id][1][0]) < 1e-7 && no->value[id][0][1] > no->value[id][1][1])) {
            for (int i=0; i<2; ++i) {
                swap(no->value[id][0][i], no->value[id][1][i]);
                swap(no->sno[id][0][i], no->sno[id][1][i]);
            }
            swap(no->no[id][0], no->no[id][1]);
        }
    } else if (no->value[id][0][0] > v) {
        no->no[id][1] = no->no[id][0];
        no->value[id][1][0] = no->value[id][0][0];
        no->sno[id][1][0] = no->sno[id][0][0];
        no->value[id][1][1] = no->value[id][0][1];
        no->sno[id][1][1] = no->sno[id][0][1];

        no->value[id][0][0] = v;
        no->sno[id][0][0] = j;
        no->no[id][0] = i;
        no->sno[id][0][1] = n + 2;
        no->value[id][0][1] = 1e12;
    } else if (no->value[id][1][0] >= v) {
        no->value[id][1][0] = v;
        no->sno[id][1][0] = j;  
        no->no[id][1] = i;
        no->sno[id][1][1] = n + 2; 
        no->value[id][1][1] = 1e12;
    }
}
...
void make_and_solve_graph_L2(const vector<double>& mu) {
        ...
        for (int i=1; i<=n; ++i) {
            ...
            for (int j=0; j<(int)at[t - p[i]].size(); ++j) {
                ...
                if (dji < dij || (dji == dij && od[jt->i] < od[i])) {
                    ...
                    if (y == 1) {
                        if (jt->value[0][0][1] > jt->value[0][1][0]) {
                            if (jt->no[0][1] == i) {
                                x = 0;
                                y = 0;
                            } else {
                    ...
                    if (UB - 1 + EPS < jt->value[0][x][y] + H1[i][t] + smu) {
                        continue;
                    } else {    
                        update(0, no, jt->i, jt->no[0][x], jt->value[0][x][y] + f(i, t) - mu[i]);//, jt->plen[0][x] + 1);
    ...
    for (int i=0; i<(int)at[T].size(); ++i) {
        update(0, broot, at[T][i]->i, at[T][i]->no[0][0], at[T][i]->value[0][0][0]);//, at[T][i]->plen[0][0]);
    ...
    //cout << "l2 fordward lb: " << broot->value[0][0][0] + accumulate(mu.begin(), mu.end(), 0.d) << endl;
}


void L2_fordward(const vector<double>& mu) {
    //cout << "solve l2 fordward" << endl;
    for (auto& noi : at[0]) {
        for (int i=0; i<2; ++i) {
            noi->value[0][0][i] = noi->value[0][1][i] = 1e12;
            noi->sno[0][0][i] = noi->sno[0][1][i] = n + 2;
        }
        noi->no[0][0] = n + 2;
        noi->no[0][1] = n + 2;
    }
    ...
    for (int t=1; t<=T+1; ++t) {
        for (auto& noi : at[t]) {
            ...
            for (int j = noi->adj.next(0); ~j; j = noi->adj.next(j + 1)) {
                ...
                int x = noj->no[0][0] == noi->i;
                int y = x == 0 ? noj->sno[0][0][0] == noi->i : 0;
                if (y == 1) {
                    if (noj->value[0][0][1] > noj->value[0][1][0]) {
                        if (noj->no[0][1] == noi->i) {
                            x = 0;
                            y = 0;
                        } else {
                ...
                update(0, noi, noj->i, noj->no[0][x], noj->value[0][x][y] + f(noi->i, t) - mu[noi->i]);//, noj->plen[0][noi->i == noj->no[0][0]] + 1);
...
void L2_backward(const vector<double>& mu) {
    //cout << "solve L2 backward" << endl;
    for (int t=0; t<=T+1; ++t) {
        for (auto& noi : at[t]) {
            for (int i=0; i<2; ++i) {
                noi->value[1][0][i] = noi->value[1][1][i] = 1e12;
                noi->sno[1][0][i] = noi->sno[1][1][i] = n + 2;
            }
            noi->no[1][0] = n + 2;
            noi->no[1][1] = n + 2;
        }
    ...
    for (int t=T+1; t>0; --t) {
        for (auto& noi : at[t]) {
            for (int j = noi->adj.next(0); ~j; j = noi->adj.next(j + 1)) {
                //assert(noi->adj.at(j));
                auto noj = at[t - p[noi->i]][j];
                int x = noi->no[1][0] == noj->i;
                int y = x == 0 ? noi->sno[1][0][0] == noj->i : 0;
                if (y == 1) {
                    if (noi->value[1][0][1] > noi->value[1][1][0]) {
                        if (noi->no[1][1] == noj->i) {
                            x = 0;
                            y = 0;
                        } else {
                ...
                update(1, noj, noi->i, noi->no[1][x], noi->value[1][x][y] + f(noi->i, t) - mu[noi->i]);//, noi->plen[1][noj->i == noi->no[1][0]] + 1);
            }
        }
    }    
    ...
    //cout << "l2 backward lb: " << root->value[1][0][0] + accumulate(mu.begin(), mu.end(), 0.) << endl;
}
...
vector<int> compute_L2_solution_fordward() {
    ...
    auto it = broot;
    int t = T + 1;
    double lpth = broot->value[0][0][0], rpth = 0.;
    
    ////cout << broot->value[0][0][0] << endl;
    while (it != root) {
        ////cout << it->value[0][0][0] << endl;

        int nxt = -1;
        double val = 1e12;
        for (int i=0; i<2; ++i) {
            if (last == it->no[0][i]) continue;
            for (int j=0; j<1+(i==0); ++j) {
                //if (last == it->sno[0][i][j]) continue;
                if (val > it->value[0][i][j]) {
                    nxt = it->no[0][i];
                    val = it->value[0][i][j];
                }
            }
        ...
        //if (nxt == -1) exit(0); 
        for (int j=it->adj.next(0); ~j; j=it->adj.next(j+1)) {
            auto jt = at[t - p[cur]][j];
            if (jt->i == nxt) {
                it = jt;
                ////cout << it->no[0][0] << ' ' << it->no[0][1] << endl;
                //assert(it->i == nxt);
                break;
            }
        }
...
vector<int> compute_L2_solution_backward() {
    ...
    auto it = root;
    int t = 0;
    double lpth = root->value[1][0][0], rpth = 0.;
    while (it != broot) {

        double val = 1e12;
        int nxt = -1;
        for (int i=0; i<2; ++i) {
            if (last == it->no[1][i]) continue;
            for (int j=0; j<1+(i==0); ++j) {
                //if (last == it->sno[1][i][j]) continue;
                if (val > it->value[1][i][j]) {
                //if (abs(it->value[1][i][j] + rpth - lpth) < 1e-8) {
...
void elimine_states_L2L2(const vector<double>& mu) {
    ...
    for (int t=0; t<=T+1; ++t) {
        for (const auto& it : at[t]) {
            if (it->no[0][0] > n+1 || it->no[1][0] > n+1) {
                for (int j = it->adj.next(0); ~j; j = it->adj.next(j + 1)) {
            ...
            for (int j = it->adj.next(0); ~j; j = it->adj.next(j + 1)) {
                ...
                if (jt->no[0][0] > n+1 || jt->no[1][0] > n+1) {
                    erase_edge(it, jt, j);
                    k--;
                } else {
                    //continue;
                    int l1 = jt->no[0][0] == it->i, r1 = it->no[1][0] == jt->i;
                    //int l2 = jt->sno[0][l1][0] == it->i, r2 = it->sno[1][r1][0] == jt->i; 
                    if (UB - 1 + 1e-7 < jt->value[0][l1][0] + it->value[1][r1][0] + f(it->i, t) - mu[it->i] + smu) {
                        erase_edge(it, jt, j);
                        k--;
                    }
                }
...
tuple<vector<double>, double> subgradient_L2(vector<double>& mu, double g_0=1.2, int d_T=n, int d_S=2, double eps=0.001, double k_S=0.95, double k_E=1.3) {
    ...
    double cur = broot->value[0][0][0] + smu;
    ...
    while (true) {
        ...
        if (UB - cur + EPS < 1 || broot->no[0][0] > n+1) {
        ...
        if (cur - best > EPS || UB2 < UB) {
            ...
            cur = max(cur, root->value[1][0][0] + smu);
        ...
        update_multipliers(mu, cur, UB, sol_lb, g);
        //cout << "finish" << endl;
        smu = accumulate(mu.begin(), mu.end(), 0.);
        L2_fordward(mu);
        if (broot->no[0][0] > n + 1) {
            print_ub();
            exit(0);
        ...
        cur = broot->value[0][0][0] + smu;
        //cout << cur << endl;   
    }
    L2_fordward(best_mu);
    gmu = best_mu;
    sol_lb = compute_L2_solution_fordward();
    auto lb = broot->value[0][0][0] + accumulate(best_mu.begin(), best_mu.end(), 0.);
    return tuple<vector<double>, double>(best_mu, lb);
}
...
bool sublimeL2(const vector<int>& lb_solution) {
    ...
    for (int t=1; t<=T; ++t) {
        for (auto it : at[t]) {
            occ[it->i] += 1; //1; //it->value[0][0] + it->value[1][0] - mu[it->i];//it->adj.size();
        }
    }
        ...
        for (int t=0; t<=T+1; ++t) {
            ...
            for (int j=0; j<m; ++j) {
                ...
                for (int i=0; i<2; ++i) 
                    for (int j=0; j<2; ++j) {
                        for (int k=0; k<2; ++k) {
                            no->value[i][j][k] = it->value[i][j][k];
                            no->sno[i][j][k] = it->sno[i][j][k];
                        }
                        no->no[i][j] = it->no[i][j];
...
void solve() {
   ...
   // L2_fordward(mu);
    ...
    auto lb = broot->value[0][0][0] + accumulate(mu2.begin(), mu2.end(), 0.d);
    //calc_cost_lb(lb_sol, mu2);
    ...
    auto smu2 = accumulate(mu2.begin(), mu2.end(), 0.d);
    //cout << endl;
    int iter = 1;
    while (true) {
        if (!sublimeL2(lb_sol)) {
