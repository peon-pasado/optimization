    ...
    for (int t = 1; t <= T; ++t) {
        for (auto it : at[t]) {
            ...
            for (auto jt = it->adj[1].begin(); jt != it->adj[1].end();) {
                ...
                need4 = 0;
                seq[1] = (*jt).to->i;
                dfs(jt->to, 2, t - p[it->i] - p[jt->to->i], dom, 1, 3);                                                                                                                                                                                                                                                                                                                                                            
                if (!dom && need4) {
                    mm++;
                    ...
                    dom = 0;
                    need4 = 0;
                    dfs(jt->to, 2, t - p[it->i] - p[jt->to->i], dom, 1, 4);
                    if (!dom && need4) {
                        mm++;
                        jt->to->adj[0].erase(jt->back);
                        jt = it->adj[1].erase(jt);
    ...
    for (int t=1; t<=T+1; ++t) {
        for (auto& noi : at[t]) {
            ...
            for (const auto& noj : noi->adj[1]) {
                if (noj.to->no[0][noi->i == noj.to->no[0][0]] == -1) continue;
                update(0, noi, noj.to->i, noj.to->value[0][noi->i == noj.to->no[0][0]] + f(noi->i, t) - mu[noi->i]);
            }
        }
    }
    ...
    for (int t=T; t>=0; --t) {
        for (auto& noi : at[t]) {
            ...
            for (const auto& noj : noi->adj[0]) {
                if (noj.to->no[1][noi->i == noj.to->no[1][0]] == -1) continue;
                update(1, noi, noj.to->i, noj.to->value[1][noi->i == noj.to->no[1][0]] + f(noj.to->i, t + p[noj.to->i]) - mu[noj.to->i]);
            }
        }
    }    
