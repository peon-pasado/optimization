...
void add_edge(Node* x, Node* y) {
    x->adj[0].emplace_back(Edge{y});              
    y->adj[1].emplace_back(Edge{x});

    auto it_this = --(x->adj[0].end());
    auto it_other = --(y->adj[1].end());

    it_this->back = it_other;
    it_other->back = it_this;
}
...
int fr[5];
bool check_dominance(int s[], int t, int ty, int k) {
    for (int i = 0; i < k; ++i) fr[i] = s[i];
    if (ty) reverse(fr, fr + k);
    else for (int i=0; i<k; ++i) t -= p[fr[i]];
    int m = 1;
...
int seq[5];
bool need4;
void dfs(Node *it, int pos, int t, bool &domination, int ty, int k=4) {
    if (domination)
        return;
    if (pos == k) {
        need4 = 1;
        domination |= check_dominance(seq, t, ty, k);
        return;
    }
    ...
    for (auto &jt : it->adj[ty]) {
        if (domination) break;
        if (vis[jt.to->i]) continue;
        seq[pos] = jt.to->i;
        dfs(jt.to, pos + 1, t - (2 * ty - 1) * p[jt.to->i], domination, ty, k);
    }
    vis[it->i] = 0;
}

bool check_three(Node* x, Node* y, int t) {
    vis[y->i] = 1;
    seq[0] = y->i;
    bool dom = 0;
    need4 = 0;
    seq[1] = x->i;
    dfs(x, 2, t - p[y->i] - p[x->i], dom, 1, 3);
    vis[y->i] = 0;
    return dom || !need4;
}

bool check_four(Node* x, Node* y, int t) {
    vis[y->i] = 1;
    seq[0] = y->i;
    bool dom = 0;
    need4 = 0;
    seq[1] = x->i;
    dfs(x, 2, t - p[y->i] - p[x->i], dom, 1);
    vis[y->i] = 0;
    return dom || !need4;
}
    ...
    for (int t = 1; t <= T; ++t) {
        for (auto it : at[t]) {
            ...
            for (auto jt = it->adj[1].begin(); jt != it->adj[1].end();) {
                ...
                need4 = 0;
                seq[1] = (*jt).to->i;
                dfs(jt->to, 2, t - p[it->i] - p[jt->to->i], dom, 1);                                                                                                                                                                                                                                                                                                                                                            
                if (!dom && need4) {
                    mm++;
...
void make_and_solve_graph_L2(const vector<double>& mu) {
    ...
    for (auto it = at[T].begin(); it != at[T].end(); ++it) {
        update(0, broot, (*it)->i, (*it)->value[0][0]);
        add_edge(*it, broot);
    }

    for (int t=T+1; t>=0; --t) {
        for (auto it : at[t]) {
            if (it == root || it == broot) continue;
            if (it->adj[0].empty()) {
                for (auto jt = it->adj[1].begin(); jt != it->adj[1].end(); ) {
                    jt->to->adj[0].erase(jt->back);
                    jt = it->adj[1].erase(jt);
...
void elimine_states_L1L2(int UB, const vector<double>& mu) {
    cout << "elimine states l1 l2" << endl;
    double smu = accumulate(mu.begin(), mu.end(), 0.d);
    for (int t=T+1; t>0; --t) {
        for (auto it = at[t].begin(); it != at[t].end(); ++it) {
            for (auto jt = (*it)->adj[1].begin(); jt != (*it)->adj[1].end(); ) {
                //j <- i
                int x = (*jt).to->no[0][0] == (*it)->i;
                if (UB - 1 + EPS < jt->to->value[0][x] + H1[(*it)->i][t] + smu) {
                    jt->to->adj[0].erase(jt->back);
                    jt = (*it)->adj[1].erase(jt);
                } else {
                    ++jt;
    ...
    for (int t=1; t<=T+1; ++t) {
        for (auto& noi : at[t]) {
            ...
            for (const auto& noj : noi->adj[1]) {
                for (int k=0; k<2; ++k) {
                    if (noj.to->no[0][k] == -1) break;
                    if (noi->i != noj.to->no[0][k]) {
                        update(0, noi, noj.to->i, noj.to->value[0][k] + f(noi->i, t) - mu[noi->i]);
                        break;
                    }
                }
            }
    ...
    for (int t=T; t>=0; --t) {
        for (auto& noi : at[t]) {
            ...
            for (const auto& noj : noi->adj[0]) {
                for (int k=0; k<2; ++k) {
                    if (noj.to->no[1][k] == -1) break;
                    if (noi->i != noj.to->no[1][k]) {
                        update(1, noi, noj.to->i, noj.to->value[1][k] + f(noj.to->i, t + p[noj.to->i]) - mu[noj.to->i]);
                        break;
                    }
                }
            }
...
void elimine_states_L2L2(int UB, const vector<double>& mu) {
    cout << "elimine states l2 l2" << endl;
    auto valid = [&](Node* it) {
        if (it->no[0][0] == -1 || it->no[1][0] == -1) return 0;
        if (it->adj[0].empty() || it->adj[1].empty()) return 0;
        return 1;
    };
    ...
    for (int t=0; t<=T+1; ++t) {
        for (const auto& it : at[t]) {
            ...
            for (auto jt = it->adj[1].begin(); jt != it->adj[1].end();) {
                if (jt->to->no[0][0] == -1 || jt->to->no[1][0] == -1) {
                    jt->to->adj[0].erase(jt->back);
                    jt = it->adj[1].erase(jt);
                } else {
                    int l1 = jt->to->no[0][0] == it->i, r1 = it->no[1][0] == jt->to->i;
                    if (UB - 1 + EPS < jt->to->value[0][l1] + it->value[1][r1] + f(it->i, t) - mu[it->i] + smu) {
                        jt->to->adj[0].erase(jt->back);
                        jt = it->adj[1].erase(jt);
                    } else {
                        ++jt;
...
tuple<vector<double>, double> subgradient_L2(vector<double>& mu, double g_0=2, int d_T=n/4, int d_S=4, double eps=0.002, double k_S=0.8, double k_E=2) {
    ...
    while (true) {
        ...
        if (UB - cur + EPS < 1 || broot->no[0][0] == -1) {
            ...
            cout << "Terminanted in subgradient 2 | " << UB - cur + EPS << endl; 
            print_ub();
            exit(0);
        }
...
bool sublimeL2(const vector<int>& lb_solution) {
    ...
    new_ids.resize(min(3, (int)new_ids.size()));
