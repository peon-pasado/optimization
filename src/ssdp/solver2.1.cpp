...
void make_graph_L2(const vector<double>& mu) {
    ...
    for (int t=1; t<=T; ++t) {
        for (int i=1; i<=n; ++i) {
            ...
            for (auto jt = at[t - p[i]].begin(); jt != at[t - p[i]].end(); ++jt) {
                ...
                if (tji < tij) {
                    at[t].back()->adj[1].push_back(*jt);
                    (*jt)->adj[0].push_back(at[t].back());
                }
            }
        }
    }
    ...
    for (auto it = at[T].begin(); it != at[T].end(); ++it) {
        broot->adj[1].push_back(*it);
        (*it)->adj[0].push_back(broot);
    }
}
    ...
    for (int t=0; t<=T; ++t) {
        for (auto it = at[t].begin(); it != at[t].end(); ++it) {
            for (auto jt = (*it)->adj[0].begin(); jt != (*it)->adj[0].end(); ) {
                ...
                if (UB - 1 + EPS < (*it)->value[0][x] + H1[(*jt)->i][t+p[(*jt)->i]] + smu) {
    ...
    for (int t=T+1; t>0; --t) {
        for (auto it = at[t].begin(); it != at[t].end(); ++it) {
            for (auto jt = (*it)->adj[1].begin(); jt != (*it)->adj[1].end(); ) {
                ...
                if (UB - 1 + EPS < (*jt)->value[0][x] + H1[(*it)->i][t] + smu) {
...
void elimine_states_L2L2(int UB, const vector<double>& mu) {
    ...
    for (int t=0; t<=T+1; ++t) {
        for (auto& it : at[t]) {
            ...
            for (auto jt = it->adj[0].begin(); jt != it->adj[0].end();) {
                if ((*jt)->no[1][0] == -1 || (*jt)->no[0][0] == -1) {
                    jt = it->adj[0].erase(jt);
                } else {
                    int l1 = it->no[0][0] == (*jt)->i, r1 = (*jt)->no[1][0] == it->i;
                    if (UB - 1 + EPS < it->value[0][l1] + (*jt)->value[1][r1] + f((*jt)->i, t + p[(*jt)->i]) - mu[(*jt)->i] + smu) {             
                        jt = it->adj[0].erase(jt);
                    } else{
                        ++jt;
                    }
...
void four_sucessive_dominance() {
    ...
    for (auto t = 0; t <= T+1; ++t)
    {
        for (auto it : at[t])
        {
            ...
            for (auto jt = it->adj[0].begin(); jt != it->adj[0].end();)
            {
                if (b[it->i].count(make_pair(it, *jt))) {
                    jt = it->adj[0].erase(jt);
                }
                else
                {
    ...
    for (auto t = 0; t <= T+1; ++t) {
        for (auto it : at[t]) {
            for (auto jt = it->adj[1].begin(); jt != it->adj[1].end();) {
                if (b[it->i].count(make_pair(*jt, it))) {
                    jt = it->adj[1].erase(jt);
                } else {
                    ++jt;
                }
