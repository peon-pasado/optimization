...
//un grafico que muestre el numero de estados activos por iteracion (en la sublimation)
//el tiempo gastado en cada iteracion
...
inline void update(int id, Node* no, int i, double v){
    if (v > 1e9) return;
    if (no->no[id][0] == i) {
        if (no->value[id][0] > v) {
            no->value[id][0] = v;
